...
namespace socho_math {
	...
	int gridways(int a, int b, vector<int> &factorials, vector<int> &inverses, int mod=MOD) {
	...
	struct combinatorics {
		...
		vector<int> factorials, inverses;
		int md;
		
		combinatorics(int max_n, int mod = MOD) {
			factorials = generate_factorials(max_n + 10, mod);
			inverses = generate_inverses(factorials, mod);
			md = mod;
		}
		
		int ncr(int n, int r) {
			return socho_math::ncr(n, r, factorials, inverses, md);
		}
		
		int inverse(int n) {
			return socho_math::mod_inverse(n, md);
		}
		...
		int power(int b, int e) {
			return socho_math::power(b, e, md);
		}
...
void solve() {
	...
	for (int i=0; i<MXN; i++) dist[i] = LLONG_MAX / 2;
	...
	for (int ITER=1; ITER<=k+1; ITER++) {
		...
		for (int i=1; i<=n; i++) {
			if (dist[i] < LLONG_MAX / 2) proc.push({dist[i], i});
		}
		while (!proc.empty()) {
			auto x = proc.top();
			proc.pop();
		...
		if (ITER == k + 1) break;
		// flight stage
		// minimum distance to reach every node after ONE flight
		// min of d[j] + (i - j)^2
		// d[j] + i^2 + j^2 - 2ij
		...
		for (int i=1; i<=n; i++) {
			cht.addLine(Line(- 2 * i, i * i + dist[i]));
		}
		
		for (int i=1; i<=n; i++) {
			dist[i] = min(dist[i], cht.query(i) + i * i);
		}
