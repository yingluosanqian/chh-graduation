...
int test(int n, int m) {
    int ans = 0;
    for (int i = 0; i < m; i++) ans += __builtin_popcountll(i ^ (i + n)) % 2;
    return ans;
}
void test() {
    // for (int i = 0; i < 16; i++) cerr << __builtin_popcountll(i) % 2;
    // cerr << '\n';
    // // 0110100110010110
    // // 0110100110010110
    // cerr << test(1, 1) << '\n';
...
// return the number of (i ^ (i + n)) with odd number of bit for 0 <= i < m
// bit, carried value, strictly less or not, modulo
int n[64], m[64];
int64_t f[64][2][2][2];
...
void solve() {
    int64_t nn, mm;
    cin >> nn >> mm;
    for (int i = 0; i < 64; i++) n[i] = (nn >> i) & 1;
    for (int i = 0; i < 64; i++) m[i] = (mm >> i) & 1;
    for (int i = 0; i < 64; i++)
        for (int carry = 0; carry <= 1; carry++)
            for (int less = 0; less <= 1; less++)
                for (int parity = 0; parity <= 1; parity++) f[i][carry][less][parity] = 0;
        ...
        for (int carry = 0; carry <= 1; carry++) {
            for (int less = 0; less <= 1; less++) {
                for (int parity = 0; parity <= 1; parity++) {
                    if (f[i][carry][less][parity] == 0) continue;
                    for (int choose = 0; choose <= 1; choose++) {
                        int added = (choose + carry + n[i]) % 2;
                        int new_carry = (choose + carry + n[i]) / 2;
                        int new_less = (choose < m[i]) || ((choose == m[i]) && less);
                        int new_parity = parity ^ added ^ choose;
                        f[i + 1][new_carry][new_less][new_parity] += f[i][carry][less][parity];
                    }
                }
            }
        }
